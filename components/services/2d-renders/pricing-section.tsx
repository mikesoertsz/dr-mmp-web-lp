...
export function PricingSection() {
  const packages = getServicePackages("2d-renders");
    ...
    <Wrapper className="bg-slate-50 dark:bg-slate-950">
      <InnerWrap>
        <div className="w-full flex items-center justify-center mb-12">
          <TitleBlock
            heading="2D Renders & Virtual Staging Packages"
            body="Transform empty spaces with professional virtual staging"
            theme="light"
            orientation="center"
          />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg, index) => (
            ...
            >
              ...
              <CardFooter>
                <AddToCartButton
                  serviceType="2d-renders"
                  packageName={`2D Renders - ${pkg.name}`}
                  price={pkg.price}
        ...
        <div className="mt-12 text-center">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-8 shadow-md max-w-2xl mx-auto">
            <h3 className="text-xl font-bold mb-4">Need Custom Staging?</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              Have specific design requirements or need staging for unique
              spaces? We offer custom virtual staging solutions.
            </p>
