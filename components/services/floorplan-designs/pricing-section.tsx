...
export function PricingSection() {
  const packages = getServicePackages("floorplan-designs");
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400">
            Professional floorplans tailored to your property size and requirements
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg, index) => (
            <Card key={index} className={`relative ${pkg.popular ? 'border-blue-500 shadow-lg scale-105' : ''}`}>
              ...
              <CardFooter>
                <AddToCartButton
                  serviceType="floorplan-designs"
                  packageName={`Floorplan Designs - ${pkg.name}`}
                  price={pkg.price}
        ...
        <div className="mt-12 text-center">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-8 shadow-md max-w-2xl mx-auto">
            <h3 className="text-xl font-bold mb-4">Larger Property?</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              For properties over 500m² or complex commercial spaces, we offer custom pricing based on your specific requirements.
            </p>
