  ...
  items: [
    {
      ...
      icon: (
        <Camera
          size={24}
          className="text-stone-700 hover:text-black duration-300 ease-in-out transition"
          strokeWidth={1.5}
        />
      ),
    },
    ...
    },
    {
      ...
      description:
        "Comprehensive brochures to provide detailed property information.",
      icon: (
        <BookOpen
          size={24}
          className="text-stone-700 hover:text-black duration-300 ease-in-out transition"
    ...
    <Wrapper id="included">
      <InnerWrap className="">
        <div className="mb-8 md:mb-12">
          ...
          >
            <Preheading className="text-center">
        ...
        <motion.ul
          ref={listRef}
          className="grid grid-cols-1 md:grid-cols-3 gap-2  rounded-2xl overflow-hidden min-h-[30dvh] w-full p-2"
          initial="hidden"
        ...
        >
          {included.items.map((item, index) => (
            <motion.li
              key={index}
              className="flex items-center gap-3 p-4 hover:bg-[#E8DED3] bg-[#F7F0E8] duration-200 ease-in-out transition cursor-pointer border border-stone-300/10 hover:border-stone-300/40 rounded-lg w-full group"
              variants={{
              ...
              <div className="flex items-center justify-center p-4 aspect-video relative">
                <div className="absolute inset-0 w-full h-full flex items-center justify-center">
                  <Image
                    ...
                    width={100}
                    height={100}
                    className="opacity-60 group-hover:opacity-100 transition duration-300 ease-in-out"
                  />
                </div>
                {item.icon}
              </div>
              <div>
                <h3 className="font-medium tracking-tight text-md mb-1 font-poppins antialiased">
                  {item.title}
                </h3>
                <p className="text-xs md:text-sm max-w-lg text-stone-500 font-medium">
                  {item.description}
                </p>
              </div>
            </motion.li>
